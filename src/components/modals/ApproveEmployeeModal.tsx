...
interface ApproveEmployeeModalProps {
  request: NewEmployeeRequest | null;
  open: boolean;
  ...
  useEffect(() => {
    if (request) {
      ...
      setFormData({
        ...
        workRateValue: data.workRate.value,
        position: data.position,
        department: data.department,
        shift: data.shift,
        workHoursStart: data.workHours.start,
        workHoursEnd: data.workHours.end,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle>Approve & Complete Employee Details</DialogTitle>
        </DialogHeader>
